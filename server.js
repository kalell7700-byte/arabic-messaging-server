...
const DB_FILE =
  process.env.DB_FILE || './data/messages.db';
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error: 'أدخل اسمًا وكلمة مرور صحيحة'
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(401).json({
        error: 'الرمز أو كلمة المرور غير صحيحة'
      });
    }
...
io.on('connection', (socket) => {
    ...
    async (payload, cb = () => {}) => {
      try {
        ...
        const message = {
          ...
          sender_code: socket.user.code,
          created_at: new Date().toISOString()
        };

        const receiverSocket =
          online.get(receiver.id);
  ...
  socket.on('disconnect', () => {
    if (
      online.get(socket.user.id) === socket.id
    ) {
...
server.listen(
  ...
  () => {
    console.log(
      `Messaging server running on port ${PORT}`
    );
  }
);
